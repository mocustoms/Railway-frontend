...
import { StoreReceipt, StoreReceiptFormData, StoreReceiptItemFormData, Store, Product, User } from '../types';
...
interface StoreReceiptFormProps {
  ...
  onSubmit: (data: StoreReceiptFormData, status?: 'draft' | 'submitted') => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
  mode?: 'create' | 'edit';
...
const validationSchema = yup.object({
  ...
  request_date: yup.string().required('Receipt date is required'),
  requesting_store_id: yup.string().required('Receive to store is required'),
  requested_from_store_id: yup.string().required('Receive from store is required'),
...
const StoreReceiptForm: React.FC<StoreReceiptFormProps> = ({
  ...
  const [showMoreColumns, setShowMoreColumns] = useState(() => {
    ...
    const saved = localStorage.getItem('storeReceipt_showMoreColumns');
  ...
  const handleToggleMoreColumns = useCallback(() => {
    ...
    localStorage.setItem('storeReceipt_showMoreColumns', JSON.stringify(newState));
  }, [showMoreColumns]);
  ...
  const {
    ...
    defaultValues: {
      ...
      request_type: 'request', // Auto-filled for Store Receipt module
      priority: 'medium',
  ...
  // Reset form when storeReceipt prop changes
  useEffect(() => {
    if (storeReceipt) {
      reset({
        reference_number: storeReceipt.reference_number,
        request_date: storeReceipt.request_date ? storeReceipt.request_date.split('T')[0] : '',
        requesting_store_id: storeReceipt.requested_by_store_id,
        requested_from_store_id: storeReceipt.requested_from_store_id,
        request_type: 'request', // Always set to 'request' for Store Receipt module
        priority: storeReceipt.priority,
        expected_delivery_date: storeReceipt.expected_delivery_date ? storeReceipt.expected_delivery_date.split('T')[0] : '',
        notes: storeReceipt.notes || '',
        currency_id: storeReceipt.currency_id || '',
        exchange_rate: storeReceipt.exchange_rate || 1.0,
        items: storeReceipt.storeRequestItems?.map(item => ({
  ...
  const handleProductSearch = async (term: string) => {
    ...
    try {
      ...
      if (!issuingStoreId) {
        toast.error('Please select a receiving from store first');
        return;
      }
  ...
  const handleAddProduct = (product: Product) => {
    ...
    if (existingItem) {
      toast.error('Product already added to the receipt');
      return;
    }
    ...
    const newItem: StoreReceiptItemFormData = {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">
              {mode === 'create' ? 'Create Store Receipt' : 'Edit Store Receipt'}
            </h2>
            <p className="text-sm text-gray-600">
              {mode === 'create' ? 'Receive stock from other stores or warehouses' : 'Update store receipt details'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          {fields.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Search and add products to create the receipt</p>
            </div>
          )}
        </div>
      </form>
