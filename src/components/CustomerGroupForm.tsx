...
import { CustomerGroup, CustomerGroupFormData } from '../services/customerGroupService';
  ...
  useEffect(() => {
    if (customerGroup) {
      reset({
        ...
        is_default: customerGroup.is_default,
        description: customerGroup.description || '',
        account_receivable_id: customerGroup.account_receivable_id || '',
        default_liability_account_id: customerGroup.default_liability_account_id || ''
      });
    } else {
      reset({
        group_name: '',
    ...
    <form onSubmit={handleSubmit(handleFormSubmit as any)} className="space-y-6">
          ...
          render={({ field }) => (
            <div>
              ...
              <select
                ...
                disabled={isLoading}
              >
                <option value="">Select Account Receivable (Optional)</option>
              ...
              {errors.account_receivable_id && (
                <p className="mt-1 text-sm text-red-600">{errors.account_receivable_id.message}</p>
              )}
            </div>
          )}
        />
      ...
      <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
        ...
        <Button
          ...
          loading={isLoading}
        >
          {customerGroup ? 'Update' : 'Create'} Group
        </Button>
      </div>
    </form>
  );
