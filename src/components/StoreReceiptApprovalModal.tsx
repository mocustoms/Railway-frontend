...
import { StoreReceipt, StoreRequestItem } from '../types';
import { formatCurrency } from '../utils/formatters';

interface StoreReceiptApprovalModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const StoreReceiptApprovalModal: React.FC<StoreReceiptApprovalModalProps> = ({
  isOpen,
  onClose,
  storeReceipt,
  action,
  ...
  useEffect(() => {
    if (isOpen && storeReceipt?.storeRequestItems) {
      const initialItems = storeReceipt.storeRequestItems.map(item => ({
        item_id: item.id,
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={`${isApprove ? 'Approve' : 'Reject'} Store Receipt`}
      size="xl"
    >
      <div className="space-y-6">
        ...
        <div className={`flex items-center space-x-3 p-4 rounded-lg ${
          ...
          <div>
            <h3 className={`text-lg font-medium ${
              ...
              {isApprove ? 'Approve Store Receipt' : 'Reject Store Receipt'}
            </h3>
            <p className={`text-sm ${
              isApprove ? 'text-green-700' : 'text-red-700'
            }`}>
              {isApprove 
                ? 'Review and approve the store receipt with any quantity adjustments'
                : 'Provide a reason for rejecting this store receipt'
              }
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="ml-2 font-mono">{storeReceipt.reference_number}</span>
            </div>
            <div>
              <span className="text-gray-600">Receipt Date:</span>
              <span className="ml-2">{new Date(storeReceipt.request_date).toLocaleDateString()}</span>
            </div>
            <div>
              <span className="text-gray-600">Receive To:</span>
              <span className="ml-2">{storeReceipt.requestingStore?.name || 'Unknown Store'}</span>
            </div>
            <div>
              <span className="text-gray-600">Receive From:</span>
              <span className="ml-2">{storeReceipt.issuingStore?.name || 'Unknown Store'}</span>
            </div>
            <div>
              <span className="text-gray-600">Total Items:</span>
              <span className="ml-2">{storeReceipt.total_items || 0}</span>
            </div>
            <div>
        ...
        {/* Items Table */}
        {isApprove && storeReceipt.storeRequestItems && storeReceipt.storeRequestItems.length > 0 && (
          <div>
                ...
                <tfoot className="bg-gray-50">
                  <tr>
                    ...
                    <td className="border border-gray-200 px-3 py-2 font-medium text-gray-900">
                      {(storeReceipt.storeRequestItems?.reduce((sum, item) => sum + (item.requested_quantity || 0), 0) || 0).toLocaleString('en-US')}
                    </td>
                    ...
                    <td className="border border-gray-200 px-3 py-2 font-medium text-gray-900">
                      {(() => {
                        ...
                        const totalApprovedCost = approvedItems.reduce((sum, approvedItem) => {
                          const item = storeReceipt.storeRequestItems?.find(i => i.id === approvedItem.item_id);
        ...
        <div>
          {isApprove ? (
            ...
            <Textarea
              ...
              placeholder="Please provide a reason for rejecting this store receipt..."
              rows={3}
              required
            />
          )}
          ...
          >
            {isLoading ? (
              ...
              <div className="flex items-center space-x-2">
                ...
                <span>{isApprove ? 'Approve Receipt' : 'Reject Receipt'}</span>
              </div>
            )}
          </Button>
        </div>
...
export default StoreReceiptApprovalModal;
