...
const SalesInvoicePaymentForm: React.FC<SalesInvoicePaymentFormProps> = ({
  salesInvoice,
  onClose,
  onSubmit,
  isLoading = false,
  ...
  const [paymentMethodType, setPaymentMethodType] = useState<'payment_type' | 'deposit_account' | 'loyalty_card'>('payment_type');
  const [paymentTypeId, setPaymentTypeId] = useState<string>('');
  const [depositAmount, setDepositAmount] = useState<number>(0);
  const [loyaltyPointsAmount, setLoyaltyPointsAmount] = useState<number>(0);
  const [chequeNumber, setChequeNumber] = useState<string>('');
  const [bankDetailId, setBankDetailId] = useState<string>('');
  const [branch, setBranch] = useState<string>('');
  const [currencyId, setCurrencyId] = useState<string>(salesInvoice.currencyId || '');
  const [exchangeRate, setExchangeRate] = useState<number>(salesInvoice.exchangeRateValue || 1.0);
  const [exchangeRateId, setExchangeRateId] = useState<string>(salesInvoice.exchangeRateId || '');
  const [description, setDescription] = useState<string>('');
  const [transactionDate, setTransactionDate] = useState<string>(new Date().toISOString().split('T')[0]);
  const [receivableAccountId, setReceivableAccountId] = useState<string>(salesInvoice.accountReceivableId || '');
  ...
  // Fetch data - get payment types filtered for debtor payments
  const { data: paymentTypes = [] } = useQuery({
    queryKey: ['payment-types-debtor-payments'],
    queryFn: customerDepositService.getPaymentTypesForDebtorPayments
  });
  ...
  const { data: currencies = [], isLoading: isLoadingCurrencies, error: currenciesError } = useQuery({
    ...
    enabled: isOpen && !!salesInvoice // Only fetch when modal is open and invoice is available
  });
  ...
  const customerDepositBalance = useMemo(() => {
    return salesInvoice.customer?.deposit_balance || 0;
  }, [salesInvoice.customer]);

  // Get customer loyalty points
  const customerLoyaltyPoints = useMemo(() => {
    return salesInvoice.customer?.loyalty_points || 0;
  }, [salesInvoice.customer]);
  ...
  const { data: loyaltyConfig } = useQuery({
    ...
    queryFn: async () => {
      const response = await loyaltyCardService.getLoyaltyConfig();
      return response.data;
    },
    enabled: isOpen && customerLoyaltyPoints > 0
  });

  // Calculate loyalty points redemption rate
  const redemptionRate = useMemo(() => {
    return loyaltyConfig?.redemption_rate || 100; // Default: 100 points = 1 currency unit
  }, [loyaltyConfig]);

  // Calculate loyalty points value in currency
  const loyaltyPointsValue = useMemo(() => {
    if (loyaltyPointsAmount <= 0 || redemptionRate <= 0) return 0;
    return loyaltyPointsAmount / redemptionRate;
  }, [loyaltyPointsAmount, redemptionRate]);
  ...
  // Calculate loyalty points amount from item payments when using loyalty card
  // IMPORTANT: Redemption rate is based on system currency, so we need to convert invoice currency to system currency first
  const calculatedLoyaltyPointsAmount = useMemo(() => {
    if (paymentMethodType === 'loyalty_card' && calculatedPaymentAmount > 0 && redemptionRate > 0) {
      // Convert invoice currency payment amount to system currency first
      // Then calculate loyalty points based on system currency
      // Formula: loyaltyPointsAmount = (paymentAmountInInvoiceCurrency * exchangeRate) * redemptionRate
      const paymentAmountInSystemCurrency = calculatedPaymentAmount * exchangeRate;
      return Math.floor(paymentAmountInSystemCurrency * redemptionRate);
    }
    return 0;
  }, [calculatedPaymentAmount, paymentMethodType, redemptionRate, exchangeRate]);

  // Calculate actual loyalty points to use (use what user entered in items, but validate against available)
  // Note: We use calculatedLoyaltyPointsAmount (from item payments) not capped to available points
  // The backend will validate that it doesn't exceed available points
  const actualLoyaltyPointsToUse = useMemo(() => {
    if (paymentMethodType === 'loyalty_card' && calculatedLoyaltyPointsAmount > 0) {
      // Use the calculated points from item payments (what user entered)
      // Don't cap it here - let backend validate against available points
      return calculatedLoyaltyPointsAmount;
    }
    return 0;
  }, [calculatedLoyaltyPointsAmount, paymentMethodType]);

  // Calculate actual payment amount from loyalty points (what user entered in items)
  // IMPORTANT: Redemption rate is based on system currency, so we need to convert back to invoice currency
  const actualPaymentAmountFromPoints = useMemo(() => {
    if (paymentMethodType === 'loyalty_card' && actualLoyaltyPointsToUse > 0 && redemptionRate > 0 && exchangeRate > 0) {
      // Convert loyalty points to system currency first
      // Then convert back to invoice currency
      // Formula: paymentAmountInSystemCurrency = loyaltyPoints / redemptionRate
      //          paymentAmountInInvoiceCurrency = paymentAmountInSystemCurrency / exchangeRate
      const paymentAmountInSystemCurrency = actualLoyaltyPointsToUse / redemptionRate;
      return paymentAmountInSystemCurrency / exchangeRate;
    }
    return 0;
  }, [actualLoyaltyPointsToUse, redemptionRate, paymentMethodType, exchangeRate]);
  
  // Calculate the maximum loyalty points that can be used (for display/warning purposes)
  const maxLoyaltyPointsToUse = useMemo(() => {
    if (paymentMethodType === 'loyalty_card' && calculatedLoyaltyPointsAmount > 0) {
      // This is the maximum that can be used (capped at available points)
      return Math.min(calculatedLoyaltyPointsAmount, customerLoyaltyPoints);
    }
    return 0;
  }, [calculatedLoyaltyPointsAmount, customerLoyaltyPoints, paymentMethodType]);

  // Calculate total payment including deposit and loyalty points
  const totalPaymentAmount = useMemo(() => {
    ...
    } else if (paymentMethodType === 'loyalty_card') {
      // For loyalty card, use the actual payment amount that can be made with available points
      // This allows partial payments when available points are less than required
      // actualPaymentAmountFromPoints is the currency value of the loyalty points used
      return actualPaymentAmountFromPoints;
    } else {
      // For payment_type, the payment amount is the sum of item payments
      return calculatedPaymentAmount;
    }
  }, [calculatedPaymentAmount, paymentMethodType, actualPaymentAmountFromPoints]);
  ...
  const balanceAfterPayment = useMemo(() => {
    const currentBalance = salesInvoice.balanceAmount || salesInvoice.totalAmount;
    const amountToUse = currentStep === 2 ? totalPaymentAmount : paymentAmount;
    return Math.max(0, currentBalance - amountToUse);
  }, [salesInvoice.balanceAmount, salesInvoice.totalAmount, paymentAmount, totalPaymentAmount, currentStep]);
  ...
  // Payment type options - filter to only show payment types allowed for debtor payments
  const paymentTypeOptions = useMemo(() => {
    if (!Array.isArray(paymentTypes)) return [];
    
    return paymentTypes
      .filter(paymentType => paymentType.used_in_debtor_payments && paymentType.is_active)
      .map(paymentType => ({
        id: paymentType.id,
  ...
  useEffect(() => {
    if (currencies.length > 0 && salesInvoice.currencyId && !currencyId) {
      // Set currency from invoice
      setCurrencyId(salesInvoice.currencyId);
      
      // Set exchange rate from invoice if available
      if (salesInvoice.exchangeRateValue) {
        setExchangeRate(salesInvoice.exchangeRateValue);
      }
      if (salesInvoice.exchangeRateId) {
        setExchangeRateId(salesInvoice.exchangeRateId);
      }
    }
  }, [currencies, salesInvoice.currencyId, salesInvoice.exchangeRateValue, salesInvoice.exchangeRateId, currencyId]);
  ...
  // Note: This effect should NOT run when receivableAccountId changes (user selection) - only when payment method or linked accounts change
  useEffect(() => {
    let accountToSet = '';
    
    if (paymentMethodType === 'deposit_account') {
      // For Account Balance, ALWAYS use invoice's receivable account for the credit entry
      // The account_balance linked account is only used for the debit entry (reducing customer deposit)
      // The credit entry must always go to Accounts Receivable
        accountToSet = salesInvoice.accountReceivableId || '';
    } else if (paymentMethodType === 'loyalty_card') {
      // For Loyalty Card, ALWAYS use invoice's receivable account for the credit entry
      // The loyalty cards account is only used for the debit entry (reducing points)
      // The credit entry must always go to Accounts Receivable
        accountToSet = salesInvoice.accountReceivableId || '';
    ...
    if (accountToSet && accountToSet !== receivableAccountId) {
      setReceivableAccountId(accountToSet);
      // Clear error if account is set
      setErrors(prev => {
        if (prev.receivableAccountId) {
          return { ...prev, receivableAccountId: '' };
        }
        return prev;
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [paymentMethodType, linkedAccounts, salesInvoice.accountReceivableId]);
  // Note: receivableAccountId is intentionally excluded from dependencies to prevent resetting user selections

  // When Account Balance or Loyalty Card is selected, ensure currency and exchange rate are set from invoice
  useEffect(() => {
    if (paymentMethodType === 'deposit_account' || paymentMethodType === 'loyalty_card') {
      // Set currency from invoice if not already set
      if (salesInvoice.currencyId && !currencyId) {
        setCurrencyId(salesInvoice.currencyId);
      }
      // Set exchange rate from invoice if not already set
      if (salesInvoice.exchangeRateValue && (!exchangeRate || exchangeRate === 1.0)) {
        setExchangeRate(salesInvoice.exchangeRateValue);
      }
      if (salesInvoice.exchangeRateId && !exchangeRateId) {
        setExchangeRateId(salesInvoice.exchangeRateId);
      }
    }
  }, [paymentMethodType, salesInvoice.currencyId, salesInvoice.exchangeRateValue, salesInvoice.exchangeRateId, currencyId, exchangeRate, exchangeRateId]);

  // Clear receivable account error when account is set
  useEffect(() => {
    if (receivableAccountId && errors.receivableAccountId) {
      setErrors(prev => {
        const updated = { ...prev };
        delete updated.receivableAccountId;
        return updated;
      });
    }
  }, [receivableAccountId, errors.receivableAccountId]);
  ...
  const handlePaymentMethodTypeChange = (type: 'payment_type' | 'deposit_account' | 'loyalty_card') => {
    setPaymentMethodType(type);
    
    // Clear all payment-related fields
    setPaymentTypeId('');
    setDepositAmount(0);
    setLoyaltyPointsAmount(0);
    ...
    if (type === 'deposit_account') {
      ...
      // The account_balance linked account is only used for the debit entry (reducing customer deposit)
      // The credit entry must always go to Accounts Receivable
        newReceivableAccountId = salesInvoice.accountReceivableId || '';
    ...
    setReceivableAccountId(newReceivableAccountId);
    
    // Clear errors (including receivableAccountId if account is set)
    setErrors(prev => {
      ...
      // Clear receivableAccountId error if account is set
      // Note: We don't clear the error here if account is empty - let validation handle it
      // This allows the user to see that they need to select a receivable account
      if (newReceivableAccountId && updated.receivableAccountId) {
        delete updated.receivableAccountId;
      }
      return updated;
    });
  ...
  const handlePaymentTypeChange = (value: string) => {
    ...
    // Note: Do NOT set receivableAccountId to payment type's default account
  ...
  useEffect(() => {
    const maxAmount = salesInvoice.balanceAmount || salesInvoice.totalAmount;
    if (paymentAmount > maxAmount) {
      setPaymentAmount(maxAmount);
    }
  }, [salesInvoice.balanceAmount, salesInvoice.totalAmount]);
  ...
  const validateStep2 = (): boolean => {
    ...
    if (salesInvoice.items && salesInvoice.items.length > 0) {
      salesInvoice.items.forEach((item: any, index: number) => {
        ...
        const itemPaidSoFar = salesInvoice.itemPaidAmounts?.[item.id] || 0;
     ...
     // IMPORTANT: Customer deposit balance is stored in system currency, so we need to convert invoice currency to system currency
     if (paymentMethodType === 'deposit_account') {
       ...
       if (totalItemPaymentsInSystemCurrency > customerDepositBalance) {
         newErrors.depositAmount = `Total payment amount (${formatCurrency(totalItemPayments, currencySymbol)}) cannot exceed available balance of ${formatCurrency(customerDepositBalance / exchangeRate, currencySymbol)}`;
         hasErrors = true;
       }
     }

     // Validate loyalty points if using loyalty card
     if (paymentMethodType === 'loyalty_card') {
       // Calculate loyalty points from item payments
       const totalLoyaltyPointsNeeded = calculatedLoyaltyPointsAmount;
       
       if (totalLoyaltyPointsNeeded <= 0) {
         hasErrors = true;
       }

       // Allow partial payments - don't block if points exceed available
       // The system will automatically use only available points
       if (customerLoyaltyPoints <= 0) {
         newErrors.loyaltyPointsAmount = 'No loyalty points available';
         hasErrors = true;
       }
     }
     ...
     if (!hasItemPayments && paymentMethodType !== 'deposit_account' && paymentMethodType !== 'loyalty_card') {
       // Error message removed as requested
       hasErrors = true;
     }
     ...
     // For account balance and loyalty card, the validation is done against available balance/points
     ...
     const maxAmount = salesInvoice.balanceAmount || salesInvoice.totalAmount;
       ...
       if (totalPaymentAmount > maxAmount + tolerance) {
         newErrors.paymentAmount = `Total payment amount (${formatCurrency(totalPaymentAmount, salesInvoice.currencySymbol)}) cannot exceed balance of ${formatCurrency(maxAmount, salesInvoice.currencySymbol)}`;
       hasErrors = true;
       }
     } else {
       // For account balance and loyalty card, validate against their respective limits
       // Account balance validation is done above (against customerDepositBalance)
       // Loyalty card validation is done above (against customerLoyaltyPoints)
  ...
  const validateStep1 = (): boolean => {
    ...
    // Currency validation - Skip if using Account Balance or Loyalty Card
    if (paymentMethodType !== 'deposit_account' && paymentMethodType !== 'loyalty_card') {
    if (!currencyId) {
    ...
    if (!transactionDate) {
      ...
      const invoiceDate = new Date(salesInvoice.invoiceDate);
      ...
      if (paymentDate < invoiceDate) {
        newErrors.transactionDate = `Transaction date cannot be earlier than invoice date (${formatDate(salesInvoice.invoiceDate)})`;
      }
    }

    // Receivable account validation
    if (!receivableAccountId) {
      if (paymentMethodType === 'loyalty_card') {
        newErrors.receivableAccountId = 'Receivable account is required for loyalty card payments. Please select an account or configure a linked account for loyalty cards.';
      } else if (paymentMethodType === 'deposit_account') {
        newErrors.receivableAccountId = 'Receivable account is required for account balance payments. Please select an account or configure a linked account for account balance.';
      } else {
        newErrors.receivableAccountId = 'Receivable account is required';
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const paymentData: InvoicePaymentData = {
      ...
      useCustomerDeposit: paymentMethodType === 'deposit_account' || undefined,
      depositAmount: paymentMethodType === 'deposit_account' ? calculatedPaymentAmount : undefined,
      useLoyaltyPoints: paymentMethodType === 'loyalty_card' || undefined,
      loyaltyPointsAmount: paymentMethodType === 'loyalty_card' ? actualLoyaltyPointsToUse : undefined,
  ...
  const currentBalance = salesInvoice.balanceAmount || salesInvoice.totalAmount;
  const currencySymbol = salesInvoice.currencySymbol || '';
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {currentStep === 1 && (
          <>
            {/* Invoice Summary and Customer Details - Only shown on Step 1 */}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{salesInvoice.invoiceRefNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Invoice Date:</span>
                    <span className="font-medium">{formatDate(salesInvoice.invoiceDate)}</span>
                  </div>
                  {salesInvoice.dueDate && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Due Date:</span>
                      <span className="font-medium">{formatDate(salesInvoice.dueDate)}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-gray-600">Total Amount:</span>
                    <span className="font-medium">{formatCurrency(salesInvoice.totalAmount, currencySymbol)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Paid Amount:</span>
                    <span className="font-medium">{formatCurrency(salesInvoice.paidAmount || 0, currencySymbol)}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{salesInvoice.customerName || 'N/A'}</span>
                  </div>
                  {salesInvoice.customerCode && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Customer Code:</span>
                      <span className="font-medium">{salesInvoice.customerCode}</span>
                    </div>
                  )}
                  {salesInvoice.customerPhone && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Phone:</span>
                      <span className="font-medium">{salesInvoice.customerPhone}</span>
                    </div>
                  )}
                  {salesInvoice.customerEmail && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Email:</span>
                      <span className="font-medium">{salesInvoice.customerEmail}</span>
                    </div>
                  )}
                  {salesInvoice.customerAddress && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Address:</span>
                      <span className="font-medium">{salesInvoice.customerAddress}</span>
                    </div>
                  )}
                  <div className="border-t border-green-300 pt-2 mt-2">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Account Balance:</span>
                      <span className={`font-medium ${(salesInvoice.customer?.account_balance || 0) < 0 ? 'text-red-600' : 'text-gray-900'}`}>
                        {formatCurrency(salesInvoice.customer?.account_balance || 0, currencySymbol)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Deposit Balance:</span>
                      <span className="font-medium text-blue-600">
                        {formatCurrency(salesInvoice.customer?.deposit_balance || 0, currencySymbol)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Loyalty Points:</span>
                      <span className="font-medium text-purple-600">
                        {salesInvoice.customer?.loyalty_points || 0}
                      </span>
                    </div>
                  </div>
                </div>
               ...
               <div className="flex-1">
                 <label htmlFor="deposit_account" className="block text-sm font-medium text-gray-700 cursor-pointer">
                   Account Balance <span className="text-gray-500 text-xs">
                     (Available: {formatCurrency(customerDepositBalance, defaultCurrency?.symbol || '')} {defaultCurrency?.symbol || 'System'}
                     {exchangeRate !== 1.0 && ` = ${formatCurrency(customerDepositBalance / exchangeRate, currencySymbol)} ${currencySymbol}`})
                   </span>
                 </label>
               </div>
             </div>
           ...
           {customerLoyaltyPoints > 0 && (
             <div className="flex items-start space-x-3">
               <input
                 ...
                 id="loyalty_card"
                 name="paymentMethod"
                 checked={paymentMethodType === 'loyalty_card'}
                 onChange={() => handlePaymentMethodTypeChange('loyalty_card')}
                 className="mt-1"
               />
               <div className="flex-1">
                 <label htmlFor="loyalty_card" className="block text-sm font-medium text-gray-700 cursor-pointer">
                   Loyalty Card <span className="text-gray-500 text-xs">(Available: {customerLoyaltyPoints} points)</span>
                 </label>
               </div>
             </div>
           )}
        ...
        {/* Currency - Hidden when using Account Balance or Loyalty Card */}
            {paymentMethodType !== 'deposit_account' && paymentMethodType !== 'loyalty_card' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <SearchableDropdown
            ...
            onChange={(value) => {
                    ...
                    if (value && value !== salesInvoice.currencyId) {
                      const currencyName = salesInvoice.currency?.code || salesInvoice.currency?.name || salesInvoice.currencySymbol || 'invoice currency';
          ...
          />
                <p className="mt-1 text-xs text-gray-500">
                  Payment must be in invoice currency: {salesInvoice.currency?.code || salesInvoice.currency?.name || salesInvoice.currencySymbol || 'N/A'}
                </p>
          {errors.currencyId && (
        ...
        {/* Exchange Rate - Hidden when using Account Balance or Loyalty Card */}
        {paymentMethodType !== 'deposit_account' && paymentMethodType !== 'loyalty_card' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div>
          ...
          <Input
            ...
            min={salesInvoice.invoiceDate ? new Date(salesInvoice.invoiceDate).toISOString().split('T')[0] : undefined}
            onChange={(e) => {
              const newDate = e.target.value;
              setTransactionDate(newDate);
              
              // Validate immediately when date changes
              if (newDate && salesInvoice.invoiceDate) {
                const invoiceDate = new Date(salesInvoice.invoiceDate);
                ...
                if (paymentDate < invoiceDate) {
                  setErrors(prev => ({ 
                    ...prev, 
                    transactionDate: `Transaction date cannot be earlier than invoice date (${formatDate(salesInvoice.invoiceDate)})` 
                  }));
            ...
            <div>
              ...
              <SearchableDropdown
                ...
                onChange={(value) => {
                  ...
                  setReceivableAccountId(accountValue);
                  // Clear error immediately when account is selected
                  setErrors(prev => {
                    const updated = { ...prev };
                    if (accountValue && updated.receivableAccountId) {
                      delete updated.receivableAccountId;
                    }
                    return updated;
                  });
                ...
                className={errors.receivableAccountId ? 'border-red-500' : ''}
              />
              {errors.receivableAccountId && (
                <p className="mt-1 text-sm text-red-600">{errors.receivableAccountId}</p>
              )}
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="overflow-x-auto">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {salesInvoice.items.map((item: any, index: number) => {
                      const itemKey = item.id || `item-${index}`;
                      const itemTotal = item.lineTotal || item.totalAmount || 0;
                  ...
                  <tfoot className="bg-gray-50">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-right text-sm font-bold text-gray-900">
                        {formatCurrency(salesInvoice.totalAmount, currencySymbol)}
                      </td>
                      <td className="px-4 py-3 text-right text-sm font-bold text-blue-600">
                        {formatCurrency(salesInvoice.paidAmount || 0, currencySymbol)}
                      </td>
                      <td className="px-4 py-3 text-right text-sm font-bold text-blue-600">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-white rounded p-3 border border-blue-100">
                        ...
                        <div className="text-sm text-gray-700">
                          <span className="font-medium text-blue-600">
                            {formatCurrency(customerDepositBalance, defaultCurrency?.symbol || '')} ({defaultCurrency?.symbol || 'System'})
                          </span>
                        </div>
                        {exchangeRate !== 1.0 && (
                          <div className="text-xs text-gray-500 mt-1">
                            = {formatCurrency(customerDepositBalance / exchangeRate, currencySymbol)} ({currencySymbol})
                          </div>
                        )}
                      </div>
                      ...
                      {calculatedPaymentAmount > 0 && (calculatedPaymentAmount * exchangeRate) > customerDepositBalance && (
                        <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded p-2">
                          <span className="font-medium">⚠️ Insufficient Balance: </span>
                          <span>
                            Payment amount ({formatCurrency(calculatedPaymentAmount * exchangeRate, defaultCurrency?.symbol || '')}) exceeds available balance ({formatCurrency(customerDepositBalance, defaultCurrency?.symbol || '')})
                          </span>
                        </div>
                      )}
                    </div>
                ...
                {/* Loyalty Card Balance Display */}
                    {paymentMethodType === 'loyalty_card' && (
                  <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="space-y-3">
                      {/* Loyalty Points Balance */}
                      <div className="flex justify-between items-center pb-2 border-b border-blue-200">
                        <span className="text-sm font-semibold text-gray-700">Loyalty Points Balance:</span>
                        <span className="text-lg font-bold text-blue-600">{customerLoyaltyPoints.toLocaleString()} points</span>
                      </div>

                      {/* Redemption Rate Information */}
                      <div className="bg-white rounded p-3 border border-blue-100">
                        <div className="text-xs font-medium text-gray-600 mb-2">Conversion Rate (System Currency):</div>
                        <div className="text-sm text-gray-700">
                          <span className="font-medium">{redemptionRate} points</span>
                          <span className="mx-2">=</span>
                          <span className="font-medium">1 {defaultCurrency?.symbol || defaultCurrency?.code || 'System Currency'}</span>
                        </div>
                        <div className="text-xs text-gray-500 mt-1">
                          (1 point = {formatCurrency(1 / redemptionRate, defaultCurrency?.symbol || '')})
                        </div>
                        {exchangeRate !== 1.0 && (
                          <div className="text-xs text-blue-600 mt-2 pt-2 border-t border-blue-100">
                            <span className="font-medium">Invoice Currency:</span> {formatCurrency((1 / redemptionRate) / exchangeRate, currencySymbol)} per point
                          </div>
                        )}
                      </div>
                      ...
                      <div className="bg-white rounded p-3 border border-blue-100">
                        ...
                        <div className="text-sm text-gray-700">
                          <span className="font-medium">{customerLoyaltyPoints.toLocaleString()} points</span>
                          <span className="mx-2">=</span>
                          <span className="font-medium text-blue-600">
                            {formatCurrency(customerLoyaltyPoints / redemptionRate, defaultCurrency?.symbol || '')} ({defaultCurrency?.symbol || 'System'})
                          </span>
                        </div>
                        {exchangeRate !== 1.0 && (
                          <div className="text-xs text-gray-500 mt-1">
                            = {formatCurrency((customerLoyaltyPoints / redemptionRate) / exchangeRate, currencySymbol)} ({currencySymbol})
                          </div>
                        )}
                        </div>

                      {/* Payment Calculation */}
                      {calculatedLoyaltyPointsAmount > 0 && (
                        <div className="bg-white rounded p-3 border border-blue-100 mt-2">
                          <div className="text-xs font-medium text-gray-600 mb-2">Payment Calculation:</div>
                          <div className="space-y-1 text-sm">
                            <div className="flex justify-between">
                              <span className="text-gray-600">Points Required:</span>
                              <span className="font-medium text-gray-700">
                                {calculatedLoyaltyPointsAmount.toLocaleString()} points
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Payment Amount:</span>
                              <span className="font-medium text-blue-600">
                                {formatCurrency(actualPaymentAmountFromPoints, currencySymbol)}
                              </span>
                            </div>
                          </div>
                          {calculatedLoyaltyPointsAmount > customerLoyaltyPoints ? (
                            <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded p-2 mt-3">
                              <span className="font-medium">⚠️ Insufficient Points: </span>
                              <span>You need {calculatedLoyaltyPointsAmount.toLocaleString()} points but only have {customerLoyaltyPoints.toLocaleString()} points available</span>
                              <div className="text-xs text-red-600 mt-1">
                                Please reduce the payment amount in the items above
                              </div>
                            </div>
                          ) : (
                            <div className="text-sm text-green-700 bg-green-50 border border-green-200 rounded p-2 mt-3">
                              <span className="font-medium">✓ Points to Use: </span>
                              <span>{actualLoyaltyPointsToUse.toLocaleString()} points ({formatCurrency(actualPaymentAmountFromPoints, currencySymbol)})</span>
                              {customerLoyaltyPoints - actualLoyaltyPointsToUse > 0 && (
                                <div className="text-xs text-green-600 mt-1">
                                  Remaining balance: {(customerLoyaltyPoints - actualLoyaltyPointsToUse).toLocaleString()} points ({formatCurrency((customerLoyaltyPoints - actualLoyaltyPointsToUse) / redemptionRate, currencySymbol)})
                      </div>
                              )}
                            </div>
                          )}
                    ...
                    {errors.loyaltyPointsAmount && (
                      <p className="mt-2 text-sm text-red-600">{errors.loyaltyPointsAmount}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                 ...
                 {paymentMethodType === 'loyalty_card' && actualLoyaltyPointsToUse > 0 && (
                   <div className="flex justify-between">
                     <span className="text-gray-600">Payment from Loyalty Points:</span>
                     <span className="font-medium text-indigo-600">
                       {actualLoyaltyPointsToUse} points ({formatCurrency(actualPaymentAmountFromPoints, currencySymbol)})
                     </span>
                   </div>
                 )}
                 ...
                 {errors.loyaltyPointsAmount && (
                   <div className="text-red-600 text-xs mt-1">{errors.loyaltyPointsAmount}</div>
                 )}
                <div className="flex justify-between">
...
export default SalesInvoicePaymentForm;
