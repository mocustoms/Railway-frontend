...
interface ProformaInvoiceRejectionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const ProformaInvoiceRejectionModal: React.FC<ProformaInvoiceRejectionModalProps> = ({
  isOpen,
  onClose,
  proformaInvoice,
  onConfirm,
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 font-medium">{proformaInvoice.proformaRefNumber}</span>
              </div>
              <div>
                <span className="text-gray-600">Customer:</span>
                <span className="ml-2 font-medium">{proformaInvoice.customerName || 'N/A'}</span>
              </div>
              <div>
                <span className="text-gray-600">Date:</span>
                <span className="ml-2 font-medium">{proformaInvoice.proformaDate}</span>
              </div>
              <div>
                <span className="text-gray-600">Total Amount:</span>
                <span className="ml-2 font-medium">
                  {proformaInvoice.currencySymbol || ''}{proformaInvoice.totalAmount?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}
                </span>
              </div>
            </div>
          </div>
...
export default ProformaInvoiceRejectionModal;
