...
const transformReturnReason = (data: any): ReturnReason => {
  return {
    id: data.id,
    code: data.code,
    name: data.name,
...
export const returnReasonService = {
  // Get all return reasons with pagination, filtering, and sorting
  getReturnReasons: async (
    page: number = 1,
    limit: number = 10,
    filters?: ReturnReasonFilters,
    sortConfig?: ReturnReasonSortConfig
  ): Promise<ReturnReasonResponse> => {
    ...
    const response = await api.get(`/return-reasons?${params}`);
    return {
      data: response.data.data.map(transformReturnReason),
      pagination: response.data.pagination
    };
  },

  // Get return reason by ID
  getReturnReason: async (id: string): Promise<ReturnReason> => {
    const response = await api.get(`/return-reasons/${id}`);
    return transformReturnReason(response.data);
  },

  // Get all active return reasons for dropdowns
  getAllReturnReasons: async (): Promise<ReturnReason[]> => {
    const response = await api.get('/return-reasons/all');
    return response.data.map(transformReturnReason);
  },

  // Create new return reason
  createReturnReason: async (data: Partial<ReturnReason>): Promise<ReturnReason> => {
    const response = await api.post('/return-reasons', data);
    return transformReturnReason(response.data);
  },

  // Update return reason
  updateReturnReason: async (id: string, data: Partial<ReturnReason>): Promise<ReturnReason> => {
    const response = await api.put(`/return-reasons/${id}`, data);
    return transformReturnReason(response.data);
  },

  // Delete return reason
  deleteReturnReason: async (id: string): Promise<void> => {
    await api.delete(`/return-reasons/${id}`);
  },

  // Get return reason statistics
  getReturnReasonStats: async (): Promise<ReturnReasonStats> => {
    const response = await api.get('/return-reasons/stats/overview');
  ...
  exportToExcel: async (filters?: ReturnReasonFilters): Promise<Blob> => {
    ...
    const response = await api.get(`/return-reasons/export/excel?${params}`, {
      responseType: 'blob'
    });
  ...
  exportToPDF: async (filters?: ReturnReasonFilters): Promise<Blob> => {
    ...
    const response = await api.get(`/return-reasons/export/pdf?${params}`, {
      responseType: 'blob'
    });
    return response.data;
  }
