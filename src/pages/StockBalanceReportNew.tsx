...
const StockBalanceReport: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleExport = (format: 'excel' | 'pdf') => {
    // Exporting stock balance report
  ...
  return (
    <div className="space-y-6">
      ...
      {stockBalanceData && stockBalanceData.length > 0 && (
        <Card title="Stock Balance Results" className="p-6">
          ...
          <div className="overflow-auto max-h-[600px] border border-gray-200 rounded-lg custom-scrollbar">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gradient-to-r from-blue-50 to-indigo-50 sticky top-0 z-10 border-b-2 border-blue-200">
                <tr>
                  {visibleColumns.productCode && (
                    <th 
                      className="px-6 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider cursor-pointer hover:bg-blue-100 hover:text-blue-800 select-none transition-colors duration-200"
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortData(stockBalanceData).map((item) => (
                  ...
                  <tr>
                    ...
                    {visibleColumns.quantity && (
                      <td className="px-6 py-3 text-base font-bold text-gray-900 text-right">
                        <strong>{formatNumber(stockBalanceData.reduce((sum, item) => sum + (item.quantity || 0), 0))}</strong>
                      </td>
                    )}
                    {visibleColumns.unitCost && (
                    ...
                    {visibleColumns.totalValue && (
                      <td className="px-6 py-3 text-base font-bold text-gray-900 text-right">
                        <strong>{formatCurrencySafe(stockBalanceData.reduce((sum, item) => sum + (item.totalValue || 0), 0))}</strong>
                      </td>
                    )}
                    {visibleColumns.lastUpdated && (
                      <td className="px-6 py-3 text-sm font-bold text-gray-900">
