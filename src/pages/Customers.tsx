...
const Customers: React.FC = () => {
  ...
  const [customerGroups, setCustomerGroups] = React.useState<any[]>([]);
  const [loyaltyCards, setLoyaltyCards] = React.useState<any[]>([]);
  ...
  const openDeleteModal = useCallback((customer: any) => {
  ...
  const columns = useMemo(() => [
    ...
    {
      key: 'account_receivable_name',
      header: 'Receivable Account',
      sortable: true,
      render: (row: any) => (
        <div className="text-gray-700">{row.account_receivable_name || '-'}</div>
      )
    },
    {
      key: 'phone_number',
    ...
    },
    {
      ...
      defaultVisible: false,
      render: (row: any) => (
        <div className="text-gray-700">{row.loyalty_card_name || '-'}</div>
      )
    },
    {
      ...
      defaultVisible: false,
      render: (row: any) => (
        <div className="text-gray-700">{row.loyalty_card_number || '-'}</div>
      )
    },
    {
      key: 'address',
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="text-blue-600 font-medium">
          {row.loyalty_points !== undefined && row.loyalty_points !== null 
            ? new Intl.NumberFormat('en-US', { 
                minimumFractionDigits: 2, 
                maximumFractionDigits: 2 
              }).format(parseFloat(row.loyalty_points))
            : '-'}
        </div>
      )
    },
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-fadeIn">
      <ContentContainer>
          ...
          <Card className="animate-slideInUp hover:shadow-md transition-all duration-150 min-w-0 flex-1" animationDelay={0}>
            <div className="flex items-center justify-between">
              ...
              <div className="p-2 bg-blue-100 rounded-full flex-shrink-0 ml-3">
                {/* No specific icon for customers; reuse Users icon style if needed */}
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    a.click(); 
                    URL.revokeObjectURL(url);
                    toast.success('Customers exported to PDF successfully');
                  } catch (error) {
                    toast.error('Failed to export customers to PDF');
                  }
                }}
                disabled={isLoading || customers.length === 0}
        ...
        {/* Enhanced Customers Table Container */}
        ...
        {customers && customers.length > 0 && pagination && (
      ...
      >
        {viewing && (
          <CustomerView customer={viewing} onClose={() => setViewing(null)} />
        )}
      </Modal>
