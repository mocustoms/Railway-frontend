...
import { useBankDetailManagement } from '../hooks/useBankDetailManagement';
...
const BankDetailsPage: React.FC = () => {
  ...
  const {
    ...
    exportToPDF,
    filters
  } = useBankDetailManagement();

  // Local state
  const [showForm, setShowForm] = useState(false);
  const [editingBankDetail, setEditingBankDetail] = useState<BankDetail | undefined>(undefined);
  ...
  const handleFormSubmit = useCallback(async (data: any) => {
    try {
      if (editingBankDetail) {
        await updateBankDetail(editingBankDetail.id, data);
      } else {
        await createBankDetail(data);
      }
      setShowForm(false);
      setEditingBankDetail(undefined);
    } catch (error) {
      // Error is already handled by the hook with toast notifications
    }
  }, [editingBankDetail, createBankDetail, updateBankDetail]);

  // Handle edit
  const handleEdit = useCallback((bankDetail: BankDetail) => {
    setEditingBankDetail(bankDetail);
    setShowForm(true);
  }, []);

  // Handle delete
  const handleDelete = useCallback(async (bankDetail: BankDetail) => {
    const confirmed = await confirm({
      title: 'Delete Bank Detail',
      message: `Are you sure you want to delete "${bankDetail.bankName}"? This action cannot be undone.`,
    ...
    try {
      await deleteBankDetail(bankDetail.id);
    } catch (error) {
      // Error is already handled by the hook with toast notifications
    }
  }, [deleteBankDetail, confirm]);

  // Handle status toggle
  const handleStatusToggle = useCallback(async (bankDetail: BankDetail) => {
    const action = bankDetail.is_active ? 'deactivate' : 'activate';
    const confirmed = await confirm({
      title: `${action.charAt(0).toUpperCase() + action.slice(1)} Bank Detail`,
      message: `Are you sure you want to ${action} "${bankDetail.bankName}"?`,
    ...
    try {
      await toggleBankDetailStatus(bankDetail.id);
    } catch (error) {
      // Error is already handled by the hook with toast notifications
    }
  }, [toggleBankDetailStatus, confirm]);
  ...
  const handleTableSort = useCallback((key: string, direction: 'asc' | 'desc') => {
    handleSort(key as keyof BankDetail, direction);
  }, [handleSort]);
  ...
  // Helper functions
  const getStatusBadge = (bankDetail: BankDetail) => {
    return (
      <StatusBadge
        status={bankDetail.is_active ? 'Active' : 'Inactive'}
        variant={bankDetail.is_active ? 'success' : 'error'}
      />
    );
  };

  const canDeleteBankDetail = (bankDetail: BankDetail) => {
    return canDelete && bankDetail.is_active;
  };

  const canEditBankDetail = (bankDetail: BankDetail) => {
    return canEdit;
  };

  const canToggleBankDetailStatus = (bankDetail: BankDetail) => {
  ...
  const columns = [
    {
      ...
      defaultVisible: true,
      sortable: true,
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900 font-medium">{bankDetail.code}</div>
      )
    },
    {
      key: 'bankName',
      ...
      sortable: true,
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">{bankDetail.bankName}</div>
      )
    },
    {
      key: 'branch',
      ...
      sortable: true,
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">{bankDetail.branch}</div>
      )
    },
    {
      key: 'accountNumber',
      ...
      sortable: true,
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900 font-mono">{bankDetail.accountNumber}</div>
      )
    },
    {
      key: 'accountId',
      ...
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">
          {bankDetail.account ? (
            <div>
              <div className="font-medium">{bankDetail.account.code}</div>
              <div className="text-gray-500 text-xs">{bankDetail.account.name}</div>
            </div>
          ) : 'N/A'}
        </div>
      )
    },
    {
      ...
      defaultVisible: true,
      sortable: true,
      render: (bankDetail: BankDetail) => getStatusBadge(bankDetail)
    },
    {
      key: 'created_at',
      header: 'Created Date',
      ...
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">
          {bankDetail.createdAt ? new Date(bankDetail.createdAt).toLocaleDateString() : 'N/A'}
        </div>
      )
    },
    {
      ...
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">
          {bankDetail.creator ? `${bankDetail.creator.first_name} ${bankDetail.creator.last_name}` : 'N/A'}
        </div>
      )
    },
    {
      ...
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">
          {bankDetail.updatedAt ? new Date(bankDetail.updatedAt).toLocaleDateString() : 'N/A'}
        </div>
      )
    },
    {
      ...
      render: (bankDetail: BankDetail) => (
        <div className="text-sm text-gray-900">
          {bankDetail.updater ? `${bankDetail.updater.first_name} ${bankDetail.updater.last_name}` : 'N/A'}
        </div>
      )
    },
    {
      ...
      render: (bankDetail: BankDetail) => (
        <div className="flex items-center justify-end space-x-2">
          ...
          {canToggleBankDetailStatus(bankDetail) && (
            <button
              onClick={() => handleStatusToggle(bankDetail)}
              className={`p-1 ${
                bankDetail.is_active
                  ? 'text-red-600 hover:text-red-900'
                  : 'text-green-600 hover:text-green-900'
              }`}
              title={bankDetail.is_active ? 'Deactivate bank detail' : 'Activate bank detail'}
            >
              {bankDetail.is_active ? <XCircle size={16} /> : <CheckCircle size={16} />}
            </button>
          )}
          {canDeleteBankDetail(bankDetail) && (
            <button
              onClick={() => handleDelete(bankDetail)}
      ...
      <ContentContainer>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 w-full">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-gray-600 truncate">Total Bank Details</p>
                  <p className="text-2xl font-bold text-gray-900">
                    {isLoadingStats ? '...' : stats?.totalBankDetails || 0}
                  </p>
                </div>
          ...
          <div className="animate-slideInUp hover:shadow-md transition-all duration-150 min-w-0 flex-1" style={{animationDelay: '100ms'}}>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-gray-600 truncate">Active Bank Details</p>
                  <p className="text-2xl font-bold text-green-600">
                    {isLoadingStats ? '...' : stats?.activeBankDetails || 0}
                  </p>
                </div>
          ...
          <div className="animate-slideInUp hover:shadow-md transition-all duration-150 min-w-0 flex-1" style={{animationDelay: '200ms'}}>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-gray-600 truncate">Inactive Bank Details</p>
                  <p className="text-2xl font-bold text-red-600">
                    {isLoadingStats ? '...' : stats?.inactiveBankDetails || 0}
                  </p>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 animate-slideInUp hover:shadow-md transition-all duration-150 mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search bank details by code, bank name, or branch..."
              value={searchTerm}
        ...
        {/* Enhanced Bank Details Table Container */}
          ...
          {isLoading ? (
            ...
            <>
              <DataTable
                data={bankDetails}
                columns={columns}
                emptyMessage="No bank details found matching your criteria."
