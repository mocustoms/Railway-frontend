...
import { useSalesInvoiceManagement } from '../hooks/useSalesInvoiceManagement';
import { SalesInvoice, SalesInvoiceSortConfig, Company } from '../types';
...
import SalesInvoicePaymentForm, { InvoicePaymentData } from '../components/SalesInvoicePaymentForm';
import ConfirmDialog from '../components/ConfirmDialog';
import SalesInvoiceRejectModal from '../components/SalesInvoiceCancelModal';
import SalesInvoiceCancelModal from '../components/SalesInvoiceCancelModal';
...
const SalesInvoices: React.FC = () => {
  ...
  const { user } = useAuth();
  const {
    salesInvoices, stats, stores, totalItems, totalPages, currentPage, pageSize,
    isLoading, isCreating, isUpdating, isDeleting,
    isSending, isApproving, isRejecting, isCancelling,
    error,     handleSearch, handlePageChange, handlePageSizeChange,
    handleSort, handleFilter, handleManualFetch, handleDateFilterChange, createSalesInvoice, updateSalesInvoice,
    deleteSalesInvoice, sendSalesInvoice, approveInvoice, rejectSalesInvoice, cancelSalesInvoice,
    exportToExcel, exportToPDF, refetchSalesInvoices,
    canCreate, canEdit, canDelete, canExport, canSend, canApprove, canReject, canCancel,
    sortConfig, filters, searchTerm
  } = useSalesInvoiceManagement();
  ...
  const handleEdit = useCallback(async (invoice: SalesInvoice) => {
    ...
    try {
      ...
      const completeInvoice = await salesInvoiceService.getSalesInvoice(invoice.id);
      setSelectedSalesInvoice(completeInvoice);
      setModalMode('edit');
      setIsModalOpen(true);
    } catch (error) {
  ...
  const handleView = useCallback(async (invoice: SalesInvoice) => {
    try {
  ...
  const handleDelete = useCallback((invoice: SalesInvoice) => {
    setSelectedSalesInvoice(invoice);
    setIsDeleteModalOpen(true);
  }, []);

  const handleSend = useCallback((invoice: SalesInvoice) => {
    setSelectedSalesInvoice(invoice);
    setActionType('send');
    setIsActionModalOpen(true);
  }, []);

  const handleApprove = useCallback((invoice: SalesInvoice) => {
    setSelectedSalesInvoice(invoice);
    setActionType('approve');
    setIsActionModalOpen(true);
  }, []);

  const handleReject = useCallback((invoice: SalesInvoice) => {
  ...
  const handlePayInvoice = useCallback(async (invoice: SalesInvoice) => {
    try {
      // Fetch the complete invoice with all associations
      const completeInvoice = await salesInvoiceService.getSalesInvoice(invoice.id);
      setSelectedSalesInvoice(completeInvoice);
  ...
  const handleRecordPayment = useCallback(async (paymentData: InvoicePaymentData) => {
    ...
    try {
      ...
      const updatedInvoice = await salesInvoiceService.recordPayment(selectedSalesInvoice.id, paymentData);
  ...
  }, [selectedSalesInvoice, refetchSalesInvoices]);
  ...
  const handleConfirmDelete = async () => {
    if (selectedSalesInvoice) {
      await deleteSalesInvoice(selectedSalesInvoice.id);
  ...
  const handleConfirmAction = async () => {
    if (selectedSalesInvoice) {
      switch (actionType) {
        case 'send':
          await sendSalesInvoice(selectedSalesInvoice.id);
          break;
        case 'approve':
          await approveInvoice(selectedSalesInvoice.id);
          break;
      }
  ...
  const handleConfirmReject = async (rejectionReason: string) => {
    if (selectedSalesInvoice) {
      await rejectSalesInvoice(selectedSalesInvoice.id, rejectionReason);
      setIsRejectModalOpen(false);
      setSelectedSalesInvoice(null);
    }
  };

  const handleCancel = useCallback((invoice: SalesInvoice) => {
  ...
  const handleConfirmCancel = async (cancellationReason: string) => {
    if (selectedSalesInvoice) {
      await cancelSalesInvoice(selectedSalesInvoice.id, cancellationReason);
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    } else if (selectedSalesInvoice) {
      await updateSalesInvoice(selectedSalesInvoice.id, data);
    }
    setIsModalOpen(false);
    setSelectedSalesInvoice(null);
  };
  ...
  const getStatusActions = useCallback((invoice: SalesInvoice) => {
    return (
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      key: 'actions',
      header: 'Actions',
      render: (invoice: SalesInvoice) => getStatusActions(invoice),
      defaultVisible: true
    }
  ], [getStatusActions]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-500 text-xl mb-4">Error loading sales invoices</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-fadeIn">
        ...
        <div className="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200 animate-slideInUp hover:shadow-lg transition-all duration-150">
          {isLoading ? (
            <div className="flex items-center justify-center py-12 animate-pulse">
              ...
              <span className="ml-2 text-gray-600">Loading sales invoices...</span>
            </div>
          ) : (
            <>
              <DataTable
                ...
                onSort={(key, direction) => {
                  ...
                  const sortFieldMap: Record<string, SalesInvoiceSortConfig['field']> = {
                  ...
                  const sortField = sortFieldMap[key] || key as SalesInvoiceSortConfig['field'];
                ...
                initialSortState={{
                  ...
                  key: (() => {
                    const reverseMap: Record<SalesInvoiceSortConfig['field'], string> = {
                      'invoiceRefNumber': 'invoiceRefNumber',
                      'invoiceDate': 'invoiceDate',
                      'customerName': 'customerName',
                      'storeName': 'storeName',
                ...
                getRowClassName={(invoice: SalesInvoice) => {
      ...
      >
        {selectedSalesInvoice && (
          <SalesInvoiceView
            salesInvoice={selectedSalesInvoice}
            onEdit={
              canEdit && selectedSalesInvoice.status !== 'approved' && 
              selectedSalesInvoice.status !== 'paid' && 
              selectedSalesInvoice.status !== 'cancelled'
                ? () => {
              setIsViewModalOpen(false);
              handleEdit(selectedSalesInvoice);
                  }
                : undefined
            }
          />
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete "${selectedSalesInvoice?.invoiceRefNumber}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        variant="danger"
      ...
      <ConfirmDialog
        ...
        title={`${actionType === 'send' ? 'Send' : 'Approve'} Sales Invoice`}
        message={`Are you sure you want to ${actionType === 'send' ? 'send' : 'approve'} "${selectedSalesInvoice?.invoiceRefNumber}"?`}
