...
import { useVendorGroupManagement } from '../hooks/useVendorGroupManagement';
import { VendorGroup } from '../services/vendorGroupService';
import './VendorGroups.css';

const VendorGroups: React.FC = () => {
  ...
  const {
    ...
    exportToExcel,
    exportToPdf
  } = useVendorGroupManagement();

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingVendorGroup, setEditingVendorGroup] = useState<VendorGroup | null>(null);
  const [deletingVendorGroup, setDeletingVendorGroup] = useState<VendorGroup | null>(null);
  const [viewingVendorGroup, setViewingVendorGroup] = useState<VendorGroup | null>(null);
  ...
  const handleFormSubmit = async (data: any) => {
    if (editingVendorGroup) {
      await updateVendorGroup(editingVendorGroup.id, data);
    } else {
  ...
  const handleView = (vendorGroup: VendorGroup) => {
    setViewingVendorGroup(vendorGroup);
  };

  const handleDelete = async (vendorGroup: VendorGroup) => {
    try {
      const usage = await checkVendorGroupUsage(vendorGroup.id);
      if (usage.isUsed) {
        alert(`Cannot delete this vendor group as it is being used by ${usage.usageCount} vendor(s).`);
        return;
      }
      setDeletingVendorGroup(vendorGroup);
      } catch (error) {
  ...
  const confirmDelete = async () => {
    if (deletingVendorGroup) {
      await deleteVendorGroup(deletingVendorGroup.id);
      setDeletingVendorGroup(null);
    }
  };

  const handleStatusToggle = async (vendorGroup: VendorGroup) => {
    await toggleVendorGroupStatus(vendorGroup.id);
  };
  ...
  const columns = useMemo(() => [
    ...
    {
      key: 'description' as keyof VendorGroup,
      header: 'Description',
      render: (vendorGroup: VendorGroup) => (
    ...
    {
      key: 'is_active' as keyof VendorGroup,
      header: 'Status',
      sortable: true,
      render: (vendorGroup: VendorGroup) => (
        <StatusBadge 
          status={vendorGroup.is_active ? 'active' : 'inactive'}
        />
      )
    },
    {
      key: 'account_payable_name' as keyof VendorGroup,
      header: 'Account Payable',
      sortable: true,
      defaultVisible: false,
      render: (vendorGroup: VendorGroup) => (
        <div className="text-sm text-gray-600">
          {vendorGroup.account_payable_name || '-'}
        </div>
      )
    },
    {
      key: 'default_liability_account_name' as keyof VendorGroup,
      header: 'Liability Account',
      sortable: true,
      defaultVisible: false,
      render: (vendorGroup: VendorGroup) => (
        <div className="text-sm text-gray-600">
          {vendorGroup.default_liability_account_name || '-'}
        </div>
      )
    },
    {
      key: 'created_at' as keyof VendorGroup,
      header: 'Created',
      sortable: true,
      render: (vendorGroup: VendorGroup) => (
        <div className="text-sm text-gray-600">
          {new Date(vendorGroup.created_at).toLocaleDateString()}
        </div>
      )
    },
    {
      key: 'updated_at' as keyof VendorGroup,
      header: 'Updated',
      sortable: true,
      hidden: true,
      render: (vendorGroup: VendorGroup) => (
        <div className="text-sm text-gray-600">
          {new Date(vendorGroup.updated_at).toLocaleDateString()}
        </div>
      )
    },
    {
      key: 'created_by' as keyof VendorGroup,
      header: 'Created By',
      sortable: true,
      hidden: true,
      render: (vendorGroup: VendorGroup) => (
            ...
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                <span className="text-xs font-medium text-blue-600">
                  {vendorGroup.creator.first_name?.charAt(0)}{vendorGroup.creator.last_name?.charAt(0)}
                </span>
              </div>
              <div className="font-medium">{vendorGroup.creator.first_name} {vendorGroup.creator.last_name}</div>
            </div>
          ) : (
    ...
    {
      key: 'updated_by' as keyof VendorGroup,
      header: 'Updated By',
      sortable: true,
      defaultVisible: false,
      render: (vendorGroup: VendorGroup) => (
            ...
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center">
                <span className="text-xs font-medium text-green-600">
                  {vendorGroup.updater.first_name?.charAt(0)}{vendorGroup.updater.last_name?.charAt(0)}
                </span>
              </div>
              <div className="font-medium">{vendorGroup.updater.first_name} {vendorGroup.updater.last_name}</div>
            </div>
          ) : (
            <span className="text-gray-400">-</span>
          )}
    ...
    },
    {
      ...
      render: (vendorGroup: VendorGroup) => (
        <div className="flex items-center justify-end space-x-2">
          ...
          {canUpdate && (
            <button
              onClick={() => handleEdit(vendorGroup)}
          ...
          {canDelete && (
            <button
              onClick={() => handleDelete(vendorGroup)}
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-fadeIn">
      <ContentContainer>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 animate-slideInUp hover:shadow-md transition-all duration-150 mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search vendor groups by name or code..."
              value={searchTerm}
              ...
              {canExport && (
                <>
                  <button
                    ...
                    disabled={isLoadingVendorGroups || vendorGroups.length === 0}
        ...
        {/* Enhanced Vendor Groups Table Container */}
        <div className="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200 animate-slideInUp hover:shadow-lg transition-all duration-150">
          {isLoadingVendorGroups ? (
            <div className="flex items-center justify-center py-12 animate-pulse">
              ...
              <span className="ml-2 text-gray-600">Loading vendor groups...</span>
            </div>
          ) : (
            <>
              <DataTable
                data={vendorGroups}
                columns={columns}
                emptyMessage="No vendor groups found matching your criteria."
                onSort={handleSort}
                sortable={true}
              ...
              {vendorGroups && vendorGroups.length > 0 && pagination && (
      ...
      <Modal
        ...
        title={editingVendorGroup ? 'Edit Vendor Group' : 'Add Vendor Group'}
        size="lg"
      >
        <VendorGroupForm
          vendorGroup={editingVendorGroup || undefined}
          accounts={accounts || []}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Vendor Group Details</h3>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setViewingVendorGroup(null)}
              >
                <X className="w-5 h-5" />
              </Button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">
                  {viewingVendorGroup.is_default ? 'Yes' : 'No'}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Description</label>
                <p className="mt-1 text-sm text-gray-900">
                  {viewingVendorGroup.description || 'No description'}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Status</label>
                <StatusBadge 
                  status={viewingVendorGroup.is_active ? 'active' : 'inactive'}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Account Payable</label>
                <p className="mt-1 text-sm text-gray-900">
                  {viewingVendorGroup.account_payable_name || 'Not set'}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Liability Account</label>
                <p className="mt-1 text-sm text-gray-900">
                  {viewingVendorGroup.default_liability_account_name || 'Not set'}
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Created</label>
                <p className="mt-1 text-sm text-gray-900">
                  {new Date(viewingVendorGroup.created_at).toLocaleDateString()} by{' '}
                  {viewingVendorGroup.creator?.first_name} {viewingVendorGroup.creator?.last_name}
                </p>
              </div>
            </div>
