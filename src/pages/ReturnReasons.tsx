...
import { useReturnReasonManagement } from '../hooks/useReturnReasonManagement';
...
import ReturnReasonView from '../components/ReturnReasonView';
import ConfirmDialog from '../components/ConfirmDialog';
import ContentContainer from '../components/ContentContainer';
import { formatDate } from '../utils/formatters';
import './ReturnReasons.css';
...
const ReturnReasons: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    if (selectedReturnReason) {
      await deleteReturnReason(selectedReturnReason.id);
  ...
  const handleFormSubmit = async (data: Partial<ReturnReason>) => {
    if (modalMode === 'create') {
      ...
      await updateReturnReason(selectedReturnReason.id, data);
    }
    handleModalClose();
  };
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      render: (returnReason: ReturnReason) => (
        <span className="text-gray-600">
          {returnReason.createdByUser ? (
            `${returnReason.createdByUser.first_name} ${returnReason.createdByUser.last_name}`
          ) : (
            'System'
          )}
        </span>
    ...
    },
    {
      ...
      render: (returnReason: ReturnReason) => (
        <span className="text-gray-600">
          {returnReason.updatedByUser ? (
            `${returnReason.updatedByUser.first_name} ${returnReason.updatedByUser.last_name}`
          ) : (
            'System'
          )}
        </span>
    ...
    },
    {
      ...
      render: (returnReason: ReturnReason) => (
        <span className="text-gray-600">
          {formatDate(returnReason.updatedAt)}
        </span>
      ),
      defaultVisible: false
    },
        ...
        <ContentContainer>
          <div className="p-6">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <h3 className="text-red-800 font-medium">Error loading return reasons</h3>
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-fadeIn">
      ...
      <ContentContainer>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 animate-slideInUp hover:shadow-md transition-all duration-150 mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search return reasons by name, code, or description..."
        ...
        {/* Enhanced Return Reasons Table Container */}
        <div className="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200 animate-slideInUp hover:shadow-lg transition-all duration-150">
          {isLoading ? (
            <div className="flex items-center justify-center py-12 animate-pulse">
              ...
              <span className="ml-2 text-gray-600">Loading return reasons...</span>
            </div>
          ) : (
            <>
              <DataTable
                data={returnReasons}
                columns={columns}
                emptyMessage="No return reasons found matching your criteria."
      ...
      <Modal
        ...
        title={`${modalMode === 'create' ? 'Add' : 'Edit'} Return Reason`}
        size="lg"
      >
        <ReturnReasonForm
