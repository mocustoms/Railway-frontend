...
const StockBalanceReport: React.FC = () => {
  ...
  const { data: stockBalanceData, isLoading, error } = useQuery({
    queryKey: ['current-stock-balance', queryFilters, manualFetchTrigger],
    queryFn: async () => {
      const response = await storeRequestService.getCurrentStockBalance(queryFilters);
  ...
  const handleExport = async (format: 'excel' | 'pdf') => {
    if (format === 'pdf') {
      try {
        ...
        a.href = url;
        a.download = `stock-balance-report-${new Date().toISOString().split('T')[0]}.pdf`;
